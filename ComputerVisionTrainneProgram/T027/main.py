...
if __name__ == '__main__':
    ...
    # Separa os objetos e pinta cada objeto de uma cor
    for n in range(n_objetos):
        # coordenadas de cada objeto
        Xmin, Xmax, Ymin, Ymax = 0, 0, 0, 0
        for linha in range(linhas):
            for coluna in range(colunas):
                if img_seg[linha,coluna] == n+1:
                    if Xmin == 0: # para o primeiro pixel encontrado
                        Xmin = Xmax
                    if Ymin == 0:
                        Ymin = Ymax
                    if Xmax < coluna:
        ...
        img_recort = img_edit[Ymin:Ymax, Xmin:Xmax]
        print(Ymin, Ymax, Xmin, Xmax)
        try:
            cv.imshow('Objeto' + str(n), img_recort)
            cv.waitKey(10)
        except:
            pass
