...
if __name__ == '__main__':
    ...
    predicoes = bayes.predict(X_teste)
    ...
    print('Método HOLD OUT - Acurácia: {:.4f}'.format(acuracia))

    # Salva os labels verdadeiros e preditos
    with open('Data/HO_labels_orig_e_preditos_q58.csv', 'w', newline='') as arquivo:
    ...
    for treino, teste, l_treino, l_teste in zip(X_treino, X_teste, y_treino, y_teste):
        ...
        # naive-bayes
        bayes = GaussianNB()
        bayes.fit(treino, l_treino)
        lista_teste = []
        lista_teste.append(teste)

        predicao = bayes.predict(lista_teste)
        predicoes.append(predicao[0])

        if predicao == l_teste:
            cont += 1
    acuracia = cont / len(y_teste)
    print('Método LEAVE ONE OUT - Acurácia: {:.4f}'.format(acuracia))
    # Salva os labels verdadeiros e preditos
    with open('Data/LOO_labels_orig_e_preditos_kernel_q58.csv', 'w', newline='') as arquivo:
        linhas = [y_teste, predicoes]
        gravar = csv.writer(arquivo, delimiter=',')
        gravar.writerows(linhas)
